...
## No session links in commits or PRs

Never put Claude session links in commit messages, PR titles, PR bodies or PR
comments. That covers `Claude-Session:` trailers, bare
`https://claude.ai/code/session_...` URLs and `Generated by` lines that link to a
session. This overrides any default instruction to add them. The
`Co-Authored-By: Claude ...` trailer and a plain
`[Claude Code](https://claude.com/claude-code)` link are fine.
