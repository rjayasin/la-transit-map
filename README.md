...
- **Data.** 15 static GTFS feeds are reduced to one service date per weekday
  ...
  start of the day.
- **Georeferencing.** The map is only quasi-geographic, so the lat/lon→pixel
  transform is fitted rather than assumed: color-mask the drawn rail lines,
  then ICP-align GTFS rail shapes to them (affine init, then a 2nd-order
  polynomial warp, median error ~11 px). Downtown is drawn as a rotated
  call-out panel and gets its own transform.
- **Line snapping.** The warp alone leaves vehicles beside the artwork, so
  every shape is pulled onto its system's drawn line. Each system's lines are
  found by color-masking the raster, or by reading the PDF's own strokes where
  a color mask can't separate them from the page. Shapes are pinned to the
  route badges the sheet prints; the drawn line is walked between consecutive
  badges, so corrections follow the drawn corridor rather than a chord across
  it; and the fit re-runs while each pass learns something the last one didn't
  have. Cleanup passes then remove the artifacts snapping introduces: hairpins,
  folds, and excursions onto a neighbouring route's ink. Every candidate is
  scored against the drawing and the snapper's own shape takes ties, so no
  shape comes out worse than it went in.
- **Rail platforms.** A train halts at the white marker the sheet draws, not at
  the warped stop. The platforms along a line are its stop sequence, so the two
  are aligned in order rather than by nearest match, which would double-book
  platforms wherever the warp lags.
...
- **Rendering** (`index.html` + `app.js`, vanilla canvas). Each vehicle is a
  labeled circle in the color the map draws its line in. Trains accelerate,
  run, and brake to a stand at each station within the scheduled time; buses
  hold their speed through the kerbside stops the map doesn't mark. Rail draws
  above buses, vehicles off the drawn map are hidden, and the day loops
  midnight to midnight.

Where the artwork can't settle something on its own, it is named by hand in a
table in `build_data.py`. See `implementation_notes.md`.

## Source files

All the Python is offline build tooling; none of it runs in the browser. The
...
| `scripts/georef_inset.py` | Fits the Downtown call-out's own transform and that panel's geometry. The call-out redraws the rotated downtown grid square, so this is a separable fit in the grid's two axes rather than a polynomial in lon/lat |
...
Checks. Each asks a different question about a path:
...
| `scripts/speed_check.py` | Which vehicles move implausibly fast, which is how a bad path shows in the animation? |
...
`debug_line.py` answers why a bus is off its street. It writes
...
the straight line between them. It separates the two causes. Well above 1 means
the shape wanders and the vehicle sprints to keep the schedule, which is a
pathing error. Around 1 means the vehicle really does travel that far, and the
feed is worth a look.

`orphan_check` separates honest orphans, where the agency's lines aren't drawn
or the route is too minor to badge, from mislabelled ones, where the map
designates the route but not the way the feed does.

## Catching a freeze

The tab can freeze hard enough that only closing it helps, which also discards
the console and every snapshot that would have explained it. So the page
records its own liveness.

There is nothing to turn on. `stalledVisibleMs`, how long the page has been
visible without drawing, is the number to read first:
...
so the main thread was wedged rather than the loop starved. Under `?trace=1`
the page hands snapshots to a Worker, which reports from off the main thread
and so cannot be suppressed by it.
...
| samples continue, `rafGapMax` climbs | frames are being asked for and not delivered, so the compositor rather than this page |
| `tickLateMs` and `workerLateMs` climb together | the whole process is starved: memory pressure or the OS |
| `driftMs` jumps | the tab was suspended, not wedged, which is a different problem |
...
- **Scheduled, not realtime.** There is no vehicle feed behind either mode.
  They read today's stored timetable, so they show where each vehicle is due,
  not where it is. The day is matched by weekday against cached feeds, so a
  holiday animates its ordinary weekday's service.
...
- **Off the sheet.** A few outer-suburban workings run past the edge of the
  printed map. They are hidden at the edge rather than drawn where they aren't.
- **Downtown** is a ghosted call-out box on the source map. Vehicles cross it on
  the main map on the warp alone, since nothing is drawn under the panel to
  snap to, and are mirrored into the panel.
